...
export default store => {
  store.subscribe(({ payload }) => {
    ...
    analytics.forEach(event => {
      ...
      if (!(type in lib)) {
        throw new Error(
          `[kdu-analytics:kduxs] The type "${type}" doesn't exist.`
        )
      }

      if (method && !(method in lib[type])) {
        throw new Error(
          `[kdu-analytics:kduxs] The type "${type}" has not method "${method}".`
        )
      }

      if (type === 'ecommerce' && !method) {
        throw new Error(
          `[kdu-analytics:kduxs] The type "${type}" needs to call a method. Check documentation.`
        )
      }
